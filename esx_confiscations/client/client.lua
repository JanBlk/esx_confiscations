    ...
    while true do
        ...
        for k,v in pairs(Config.Locations) do
            if (PlayerData.job and PlayerData.job.name == v.job and PlayerData.job.grade >= v.grade) or (v.job == false and v.grade == -1) then
                ...
                if distance < 2 then
                    print(Current.Action, Current.Spawn.pos, Current.Spawn.rot, Current.Garage, Current.CanTakeOut)
...
function OpenConfiscateMenu()
    Citizen.Wait(1000)
    if not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'confiscatemenu2') then
        ESX.TriggerServerCallback('RetrieveConfiscatedVehicles', function(confiscatedvehicles)
            ...
            ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'confiscatemenu2', {
                ...
                elements = elements,
                    ...
                    if Current.CanTakeOut then
                        ...
                        ESX.Notification(7500, "default", "GREEN", "[SYSTEM]", "Vehicle with plate: "..data.current.plate.." released")
                        SpawnVehicle(data.current.plate)
                        OpenConfiscateMenu()
                    else
                        ESX.Notification(7500, "default", "RED", "[SYSTEM]", "This is not a garage")
                    end
            end, function (data, menu)
                menu.close()
            end)
...
function ImpoundVehicle(vehicle)
    ...
    ESX.Notification(7500, "default", "GREEN", "[SYSTEM]", "Fahrzeug beschlagnahmt")
end
