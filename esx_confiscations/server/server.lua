...
ESX.RegisterServerCallback('RetrieveAllConfiscatedVehicles', function(source, cb)
    MySQL.Async.fetchAll('SELECT * FROM confiscations ORDER BY `date` DESC', {}, function(result)
        if result[1] ~= nil then
            for k,v in pairs(result) do
                local identifierowner = v.owner
                local identifierofficer = v.officer
